...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f'Attack started:\nFlooding {target_ip}:{port} for {duration} seconds using sharp binary, UDP flood, and Super UDP. OWNER- @ITS_KAAL_01')
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "LAUDE TERE BAP NE BANYA NA TO VO HI USE KAREGA 💀OWNER @ITS_TUFAN_01"
...
async def owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📞 Contact the owner: @ITS_KAAL_01")
